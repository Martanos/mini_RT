...
/*   Updated: 2025/01/13 17:28:55 by seayeo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	check_height(t_ray ray, double t, t_cone_object *cone)
{
	...
	t_vect normalized_normal = vect_normalize(cone->cone_normal);
...
static double	check_cone_surface(t_ray ray, t_cone_object *cone)
{
	...
	double dot_dir_axis = vect_dot(ray.direction, normalized_normal);
	double dot_oc_axis = vect_dot(oc, normalized_normal);

	a = vect_dot(ray.direction, ray.direction) - 
		(1.0 + cos_angle * cos_angle) * dot_dir_axis * dot_dir_axis;
	b = 2.0 * (vect_dot(ray.direction, oc) - 
		(1.0 + cos_angle * cos_angle) * dot_dir_axis * dot_oc_axis);
	c = vect_dot(oc, oc) - 
		(1.0 + cos_angle * cos_angle) * dot_oc_axis * dot_oc_axis;
 ...
 * @param ray The ray being cast in the scene
 * @param cone Pointer to the cone object to check intersection with
 * @return double Returns the intersection parameter t, or -1.0 if no intersection
 */
static double	check_cone_base(t_ray ray, t_cone_object *cone)
{
	t_vect normalized_normal = vect_normalize(cone->cone_normal);
	t_vect base_normal = vect_multiply(normalized_normal, -1);

	t_capped_plane base = {
		.position = cone->cone_pos,
		.normal = base_normal,
		.radius = cone->cone_diameter / 2.0
	};
...
void	calculate_cone_hit(t_ray ray, t_cone_collision collision, t_hit_record *rec)
{
	...
	t_vect cp = vect_sub(rec->point, collision.closest_cone->cone_pos);
	t_vect normalized_normal = vect_normalize(collision.closest_cone->cone_normal);
	...
	else
	{
		...
		t_vect axis_point = vect_add(collision.closest_cone->cone_pos, 
			vect_multiply(normalized_normal, height));
		t_vect radial = vect_normalize(vect_sub(rec->point, axis_point));
		double cos_angle = cos(atan(collision.closest_cone->cone_diameter / 
			(2.0 * collision.closest_cone->cone_height)));
		rec->normal = vect_normalize(vect_add(
			vect_multiply(radial, cos_angle),
