...
/*   Updated: 2025/03/06 14:49:51 by malee            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	ft_intersect_cone_base(t_ray **ray, t_obj_data *cone, t_hit **hit)
{
	...
	vec_to_center = ft_vect_sub(intersection, base_center);
	if (!ft_is_within_base(intersection, base_center, base_normal,
			cone->props.t_cone.radius))
		return (false);
	(*hit)->t = t;
	...
	(*hit)->u = (vec_to_center.x / cone->props.t_cone.radius + 1) * 0.5;
	(*hit)->v = (vec_to_center.z / cone->props.t_cone.radius + 1) * 0.5;
	return (true);
}
