...
/*   Updated: 2025/03/05 10:01:15 by malee            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_apply_bump_mapping(t_hit **hit)
{
	...
	height = ((*hit)->object->bpm.data[img_y * (*hit)->object->bpm.width
			+ img_x] & 0xFF) / 255.0;
	height_dx = ((*hit)->object->bpm.data[img_y * (*hit)->object->bpm.width
			+ ((img_x + 1) % (*hit)->object->bpm.width)] & 0xFF) / 255.0;
	height_dy = ((*hit)->object->bpm.data[((img_y + 1)
				% (*hit)->object->bpm.height) * (*hit)->object->bpm.width
			+ img_x] & 0xFF) / 255.0;
	(*hit)->normal = ft_perturb_normal(hit, height_dx - height, height_dy
			- height, 0.05);
}
